...
class DgraphClient:
    ...
    async def drop_data_only(self) -> Dict[str, Any]:
        client = await self._get_client()
        headers = {"Content-Type": "application/json"}
        payload = json.dumps({"drop_attr": "*"})
        r = await client.post(self.alter_url, content=payload, headers=headers)
        r.raise_for_status()
        return {"status": "OK", "message": "All nodes deleted, schema retained."}
